...
usersCtrl.addAuthor = async (req, res, next) => {
  try {
    ...
    for (var i = 0; i < user.followers.length; i++) {
      ...
      await User.findByIdAndUpdate(user2.id, user2, { new: true, runValidators: true })
    }
...
usersCtrl.addPublicationToAuthor = async (req, res, next) => {
  try {
    ...
    await User.findByIdAndUpdate(req.user.id, user, { new: true, runValidators: true })
    return next()
  } catch (err) {
    if (!err.message) {
      logger.warn(err)
...
usersCtrl.getAllUserCategories = async (req, res) => {
  try {
    ...
    else categories = await User.findById(req.user.id).select('subscriptionToCategories')
  ...
  try {
    ...
    for (i in followings['following']) {
      ...
      for (j in following['followers']) {
        if (following['followers'][j] == req.user.id) following.isFollowedByThisUser = 1
      }
      listFollowings.push({
        _id: following._id,
  ...
  try {
    ...
    for (i in followers['followers']) {
      ...
      for (j in follower['following']) {
        if (follower['following'][j] == req.user.id) follower.isFollowingThisUser = 1
      }
      listFollowers.push({
        _id: follower._id,
...
usersCtrl.getAllUserAuthors = async (req, res) => {
  try {
    ...
    else authors = await User.findById(req.user.id).select('subscriptionToAuthors')
...
usersCtrl.getPersonalData = async (req, res) => {
  try {
    ...
    if (req.params.userId != 'null') {
      ...
      if (user.role == 0) isFollowing = userLoggedIn.following.includes(user.id)
...
usersCtrl.startFollowing = async (req, res, next) => {
  try {
    ...
    if (req.body.categoryId) {
      ...
      if (req.body.authorId == req.user.id) throw "You can't subscribe to yourself"
    ...
    if (users.length < 10) {
      if (req.isAuthenticated()) {
        ...
        for (let i = 0; i < users.length; i++) {
          ...
          else isFollowing = userLoggedIn.subscriptionToAuthors.includes(users[i]._id)
